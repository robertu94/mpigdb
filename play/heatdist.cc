...
void exchange(size_t start, size_t stop, mds result) {
  ...
  MPI_Sendrecv(top, N, MPI_FLOAT, /*dest*/ rank ? (rank - 1) : MPI_PROC_NULL,
               /*sendtag*/ 0, tmp_bot.data(), N, MPI_FLOAT,
               /*source*/ (rank + 1) >= size ? MPI_PROC_NULL : (rank + 1),
               /*recvtag*/ 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
  MPI_Sendrecv(bottom, N, MPI_FLOAT,
               /*dest*/ (rank + 1) >= size ? MPI_PROC_NULL : (rank + 1),
               /*sendtag*/ 0, tmp_top.data(), N, MPI_FLOAT,
               /*source*/ rank ? (rank - 1) : MPI_PROC_NULL, /*recvtag*/ 0,
...
int main(int argc, char *argv[]) {
  ...
  if (rank == 0) {
    std::cout << "time: "
              << stdc::duration_cast<stdc::duration<double, std::ratio<1>>>(
                     stopt - startt)
                     .count()
              << std::endl;
  }
